...
// @include     http://old.stis.ping-pong.cz/*
// @include     https://old.stis.ping-pong.cz/*
...
function GenerateElostStatistics(elostURL, table)
{
        ...
        if (this.readyState == 4) {
            if (this.status == 200) {
                ...
                if (eloNewestYear == 0){
                    ...
                    eloNewestYear = rows[rows.length-2][CELL_ELO];
                }
                if (setYearStart && eloYearYear == 0){
                    eloYearYear = rows[rows.length-2][CELL_ELO];
                }
    ...
    xhttp.open("GET", elostURL.replace("/st/", "/api_1_0/") + "?json=yes", true);
    xhttp.send();
}
